...
use assets::Assets;
use dice::{
    DEFAULT_SET, DICE_COUNT, DiceSet, DiceState, Indices, MoveResult, MoveType, boolset_to_indices,
    result_rectangles,
};
use rand::Rng;
use raylib::prelude::*;
...
impl InterfaceState {
    ...
    pub fn frame(
        ...
        if let Some(turn) = &mut self.turn {
            ...
            self.turn = Some(DiceTurn::new_round(self.dice_set, rng));
...
fn main() -> eyre::Result<()> {
    ...
    while !rl.window_should_close() {
        ...
        let time = d.get_time();
        state.frame(&d, &assets, &mut rng, time);
        ...
        if let Some(turn) = &state.turn {
            let turn_elapsed = time - state.round_start;
            ...
            for (i, rect) in result_rectangles(&state.dice_set, &turn.dice_state.current_roll)
                .into_iter()
                .enumerate()
            {
                if state.anim_rolling.contains(&i) {
                    draw_die(
                        &mut dd,
                        &assets,
                    ...
                    let reveal_elapsed = time - state.anim_reveal[i];

                    let hover = state.anim_hover[i]
                        .map_or(0.0, |time_start| 0.5_f64.powf((time - time_start) * 16.0));

                    let (anim_shake_x, anim_shake_y) =
                        state.anim_shakes[i].map_or((0.0, 0.0), |time_start| {
                    ...
                    let anim_oscillate_y = state.anim_oscillates[i].map_or(0.0, |time_start| {
                        let elapsed = time - time_start;
                    ...
                    let x_offset =
                        (rng.random_range(-4.0..4.0) * 0.5_f64.powf(reveal_elapsed * 4.0)).round()
                            + state.anim_select[i].map_or(0.0, |_| rng.random_range(-1.0..1.0))
                            + anim_shake_x;
                    ...
                    let y_offset =
                        (rng.random_range(-4.0..4.0) * 0.5_f64.powf(reveal_elapsed * 4.0)).round()
                            ...
                            + anim_shake_y
                            + anim_oscillate_y
                            - state.anim_select[i].map_or(0.0, |time_start| {
                    ...
                    let squish_in = (0.5_f64.powf(reveal_elapsed * 5.0) * 0.2)
                        * if !matches!(state.anim_color[i], DiceDisplay::NonScoring) {
                            1.0
                        } else {
                            -0.5
                        };
                    ...
                    let squish = f64::clamp(
                        1.0 - squish_in
                            ...
                            + state.anim_deselect[i].map_or(0.0, |time_start| {
                    ...
                    draw_die(
                        ...
                        &rect,
                        state.anim_color[i].get_color(time, i),
                        Color::WHITE,
                        squish as f32,
                    );
                }
            ...
            let score = state.anim_score.ceil() as u32;
            if state.animating || turn.move_result.move_options.is_some() {
                for i in 0..4 {
            ...
            let turn_end_elapsed = (!state.animating).then_some(time - state.anim_turn_start);

            for (index, plus_score, start_time) in &state.anim_scores {
